...
const EventCard = ({event, isSelected=false}) => {
    ...
    return(
        <div onClick={()=>{navigate('/event', {state:{event: event}})}} className={`cursor-pointer w-[200px] relative border rounded-xl flex flex-col gap-2 justify-left  ${isSelected?'border-red-700' : 'border-[#FFFFFF26]'}`}>
