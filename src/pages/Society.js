...
const Society = () => {
  ...
  return (
    <div className='bg-darkGray min-h-screen flex flex-col gap-6 items-center'>
        ...
        <main className="w-8/12 flex flex-col gap-7 items-center">
            {/* banner */}
            <div className='w-full h-36'>
                <img src={society?.banner} className='object-cover w-full h-full rounded-2xl'/>
            </div>

            {/* title div */}
            <div className='flex gap-7 w-full'>
                {/* icon */}
                <div className='h-36 w-36 rounded-full bg-darkGray'>
                    <img src={society?.logo} className='rounded-full h-full w-full object-cover'/>
                </div>
                {/* title etc*/}
                <div className='flex flex-col w-1/3 gap-1'>
                    <h1 className='text-white text-3xl font-bold'>{society?.name}</h1>
            ...
            <div className={`bg-lightGray w-full rounded-2xl`}>
                <div className='py-4 px-6'>
                    ...
                    <p className='text-lightestGray'>{aboutIsExpanded? society?.about : society?.about?.substring(0,450)}<span className={`text-yellow-400 cursor-pointer ${society?.about?.length < 451? 'hidden' :  ''}`} onClick={()=>{setAboutIsExpanded((aboutIsExpanded)=>!aboutIsExpanded)}}>{aboutIsExpanded? ' ...less' : ' more...'}</span></p>
                </div>
            </div>
            ...
            <div className='bg-lightGray w-full rounded-2xl'>
                <div className='py-8 px-12'>
                    ...
                    <div className='flex justify-between items-center'>
                        <h3 className='text-white text-2xl font-semibold'>Recruitment Process</h3>
                        <button className='bg-darkGray border border-white py-2 px-10 rounded-lg cursor-pointer text-white text-lg'>Register</button>
                    </div>
                            ...
                            return(
                                <div className='relative h-full flex mt-[2px]' key={index}>
                                    ...
                                    <div className='mx-5 mb-4'>
                                        ...
                                        <p className='text-lightestGray'>{step}</p>
                                    </div>
                                </div>
                            )
                        })}
